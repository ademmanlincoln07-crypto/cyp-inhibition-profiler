#!/usr/bin/env python3
"""
Very-fast end-to-end pipeline using sensible default hyperparameters (no inner
Optuna) for 5-fold scaffold CV — scientifically valid because:
- Default LGBM/XGB/HGB parameters used here are well-established strong
  settings for ADMET QSAR on 2048-bit fingerprints.
- We still do proper scaffold CV, proper leak-free feature processing,
  proper ST-RAE-based ensemble weighting on OOF, and multi-task training.
- We still report all metrics and produce full submission/artifacts.
Inner Optuna HPO is available in train_pipeline_fast.py for final-polish runs.
"""
...
# Sensible default params — no tuning; keeps CV fast.
DEFAULT_PARAMS = {
...
def cv_cyp_fixed(cyp, df, fp, des, dn, mn, fs):
    """cv_cyp wrapper that overrides Optuna tuning with fixed strong defaults."""
    # We achieve this by temporarily monkey-patching optuna_tune in models
    from src import models as _M
    saved = _M.optuna_tune
    def _fixed_tune(*a, **k): return dict(DEFAULT_PARAMS[(mn,fs)])
    _M.optuna_tune = _fixed_tune
    try:
        r = cv_cyp(cyp, df, fp, des, dn, model_name=mn, feature_set=fs,
...
def main():
    ...
    print("="*70); print("OpenADMET CYP Pipeline (fast defaults, scaffold CV, ST-RAE ensemble)"); print("="*70)
    ...
    print("\n[2] Morgan FP (r=2, 2048, chiral) + RDKit 2D descriptors")
    ...
    print("\n[3] Per-CYP single-task models (5-fold scaffold CV, fixed strong params)")
    ...
    print("\nMacro-average across CYPs (lower ST-RAE is better):"); print(macro.to_string())
    macro.to_csv(f"{C.RESULTS_DIR}/baseline_macro_summary.csv")

    print("\n[4] Multi-task MLP (shared encoder, 4 heads, masked MSE)")
    mt_cv = cv_multitask(df_tr,fp_tr,des_tr,dn_all,feature_set="both",n_outer=5,
    ...
    print("\n[5] ST-RAE-optimal per-CYP ensemble weights (on OOF)")
    weights={}
    for j,cyp in enumerate(C.CYP_ISOFORMS):
        ...
        preds={}
        for mn,fs in spec:
            preds[f"{mn}_{fs}"]=results[(cyp,mn,fs)].oof_preds
        # align MT oof to labelled slice
        ...
        for pos,nm in enumerate(mt_cv["df_labelled"]["Molecule_Name"].values):
            if nm in n2p and np.isfinite(mt_cv["oof"][pos,j]):
                mt_a[n2p[nm]]=mt_cv["oof"][pos,j]
        mt_a[~np.isfinite(mt_a)]=y_mean
        preds["multitask"]=mt_a
        ...
        print(f"  {cyp}: best single ST-RAE={best_single:.3f} -> ensemble ST-RAE={bm['ST_RAE']:.3f}")
        print(f"    weights: {w}")
    _json(weights,f"{C.RESULTS_DIR}/ensemble_weights.json")

    print("\n[6] Refitting final models + multi-task on ALL training data")
    mt_final, _, _ = fit_final_multitask(df_tr,fp_tr,des_tr,dn_all,feature_set="both",
        device=DEVICE,seed=SEED,enc_hidden=512,enc_latent=256,head_hidden=64,dropout=0.25,
    ...
    print("\n[7] Blinded-test predictions")
    def _pred(r,fp_te,des_te):
        return r.final_feature_builder.transform(fp_te,des_te), r.final_model
    final_preds={}
    for j,cyp in enumerate(C.CYP_ISOFORMS):
    ...
    print("\nFirst 5 predictions:"); print(sub.head())

    print("\n[8] Diagnostics and figures")
    # Pred-vs-true OOF
    fig,axes=plt.subplots(2,2,figsize=(11,10))
    for i,cyp in enumerate(C.CYP_ISOFORMS):
        ax=axes[i//2,i%2]; tgt=C.TARGET_COLS[cyp]
        cl_c=C.CI_LOW_COLS[cyp]; ch_c=C.CI_HIGH_COLS[cyp]
        lab=df_tr[tgt].notna().values
        y=df_tr.loc[lab,tgt].values.astype(float)
        cl=df_tr.loc[lab,cl_c].values.astype(float)
        ch=df_tr.loc[lab,ch_c].values.astype(float)
        preds_oof={}
        for mn,fs in spec: preds_oof[f"{mn}_{fs}"]=results[(cyp,mn,fs)].oof_preds
        cyp_names=df_tr.loc[lab,"Molecule_Name"].values; n2p={nm:i for i,nm in enumerate(cyp_names)}
        mt_a=np.full(len(y),np.nan)
        for pos,nm in enumerate(mt_cv["df_labelled"]["Molecule_Name"].values):
            if nm in n2p and np.isfinite(mt_cv["oof"][pos,i]):
                mt_a[n2p[nm]]=mt_cv["oof"][pos,i]
        mt_a[~np.isfinite(mt_a)]=y.mean(); preds_oof["multitask"]=mt_a
        w=weights[cyp]; oof_blend=sum(w[k]*preds_oof[k] for k in w if k in preds_oof)
        ws=sum(w[k] for k in w if k in preds_oof)
        if ws>0: oof_blend/=ws
        ax.scatter(y,oof_blend,s=10,alpha=0.5,color="#1f77b4")
        ax.plot([y.min(),y.max()],[y.min(),y.max()],"r--",lw=1)
        ax.set_xlabel(f"Experimental {cyp} pIC50"); ax.set_ylabel("Predicted (OOF)")
    ...
    print(f"  AD flags: high={(flags==0).sum()} med={(flags==1).sum()} low={(flags==2).sum()}")
    ...
    print(f"  Figures in figures/, models in models/")
